...
Route::middleware(['auth', 'user.active'])->group(function () {
    ...
    // ===================================
    Route::prefix('inventory')->name('inventory.')->middleware(['role:super_admin,admin,inventory'])->group(function () {

        // Dashboard
        Route::get('/', function() {
            $stats = [
                'total_warehouses' => App\Models\Warehouse::count(),
                'active_warehouses' => App\Models\Warehouse::where('is_active', true)->count(),
                'total_items' => App\Models\Item::count(),
                'total_categories' => App\Models\ItemCategory::count(),
                'total_suppliers' => App\Models\Supplier::count(),
                'low_stock_items' => App\Models\Item::whereHas('stocks', function($q) {
                    $q->selectRaw('item_id, SUM(quantity_available) as total_qty')
                      ->groupBy('item_id')
                      ->havingRaw('total_qty <= items.reorder_point');
                })->count(),
            ];

            $recentTransactions = App\Models\StockTransaction::with(['warehouse', 'item', 'performedBy'])
                ->latest()->take(10)->get();

            $warehouses = App\Models\Warehouse::with(['stocks'])->withCount('stocks')->get();

            // Analytics Data for Charts

            // 1. Transaction Trends (Last 7 days)
            $transactionTrends = App\Models\StockTransaction::selectRaw('DATE(transaction_date) as date, transaction_type, COUNT(*) as count')
            ...
            $stockValueByWarehouse = App\Models\Warehouse::with(['stocks.item'])
                ->get()
                ->map(function($warehouse) {
                    $totalValue = $warehouse->stocks->sum(function($stock) {
                        return $stock->quantity_available * ($stock->item->last_unit_price ?? 0);
                    });
                    return [
                        'name' => $warehouse->name,
                        'value' => $totalValue
            ...
            // 3. Top 10 Items by Stock Quantity
            $topItemsByStock = App\Models\Item::with('category')
                ->withSum('stocks as total_stock', 'quantity_available')
            ...
            $categoryDistribution = App\Models\ItemCategory::withCount('items')
            ...
            // 5. Monthly Transaction Summary (Last 6 months)
            $monthlyTransactions = App\Models\StockTransaction::selectRaw('DATE_FORMAT(transaction_date, "%Y-%m") as month, transaction_type, COUNT(*) as count')
        ...
        // Test Page (temporary untuk testing)
        Route::get('test', function() {
            $warehouses = App\Models\Warehouse::all();
            $items = App\Models\Item::with('category')->get();
            $categories = App\Models\ItemCategory::withCount('items')->get();
            $suppliers = App\Models\Supplier::all();
            $transactions = App\Models\StockTransaction::with(['warehouse', 'item'])->latest()->take(10)->get();

            return view('inventory.test', compact('warehouses', 'items', 'categories', 'suppliers', 'transactions'));
        })->name('test');
        ...
        // Stock Transactions
        Route::resource('transactions', StockTransactionController::class)->except(['edit', 'update']);
        Route::post('transactions/stock-in', [StockTransactionController::class, 'stockIn'])->name('transactions.stock-in');
        Route::post('transactions/stock-out', [StockTransactionController::class, 'stockOut'])->name('transactions.stock-out');
        Route::post('transactions/transfer', [StockTransactionController::class, 'transfer'])->name('transactions.transfer');
        Route::post('transactions/adjustment', [StockTransactionController::class, 'adjustment'])->name('transactions.adjustment');
        ...
        // Reports
        Route::get('reports/stock-summary', [StockTransactionController::class, 'stockSummary'])->name('reports.stock-summary');
        Route::get('reports/low-stock', [StockTransactionController::class, 'lowStock'])->name('reports.low-stock');
        Route::get('reports/stock-value', [StockTransactionController::class, 'stockValue'])->name('reports.stock-value');
    });
});
