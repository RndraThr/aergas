    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <p class="text-gray-600">{{ $joint->fittingType->nama_fitting }} - {{ $joint->lokasi_joint ?: 'Lokasi belum diisi' }}</p>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">
                                ...
                                @if($joint->joint_line_optional && $joint->isEqualTee())
                                    <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded ml-2">3-Way Connection</span>
                                @endif
                            </dd>
                        </div>
                        <div>
                                ...
                                <div class="bg-gray-50 rounded-lg overflow-hidden">
                                    ...
                                    <div class="p-4">
                                        <div class="flex items-center justify-between">
                                            <h3 class="text-sm font-medium text-gray-900 capitalize">{{ str_replace('_', ' ', $photo->photo_field_name) }}</h3>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                @if($joint->tracer_approved_at)
                                    <p class="text-sm text-green-600">{{ $joint->tracer_approved_at->format('d/m/Y H:i') }}</p>
                    ...
                    @if(auth()->user()->hasAnyRole(['tracer', 'admin', 'super_admin']) && !$joint->tracer_approved_at && $joint->status_laporan !== 'revisi_tracer')
                        <div class="mt-6 space-y-2">
                            <form method="POST" action="{{ route('jalur.joint.approve-tracer', $joint) }}" class="w-full">
                                @csrf
                                <button type="submit" 
                                        onclick="return confirm('Approve laporan joint ini sebagai Tracer?')"
                    ...
                    @if(auth()->user()->hasAnyRole(['admin', 'super_admin']) && $joint->tracer_approved_at && !$joint->cgp_approved_at && $joint->status_laporan !== 'revisi_cgp')
                        <div class="mt-6 space-y-2">
                            <form method="POST" action="{{ route('jalur.joint.approve-cgp', $joint) }}" class="w-full">
                                @csrf
                                <button type="submit" 
                                        onclick="return confirm('Approve laporan joint ini sebagai CGP?')"
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-900">{{ $joint->photoApprovals ? $joint->photoApprovals->count() : 0 }} foto</p>
                        </div>
                    </div>
                </div>
            </div>
...
function openImageModal(imageUrl, title) {
    ...
    modal.innerHTML = `<div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onclick="this.remove()">
        <img src="${imageUrl}" class="max-w-full max-h-full" alt="${title}">
    </div>`;
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-lg p-6 max-w-md mx-4">
            ...
            <div class="flex space-x-3">
                ...
                <a href="https://drive.google.com/drive/folders/1HYtw0gCCxhJd9tGPRhKRggvOoOwQ42XW" target="_blank" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-center">📁 Buka Drive</a>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
