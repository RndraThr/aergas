        ...
        <div class="bg-white rounded-lg shadow mb-6 p-6">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-6 gap-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nomor joint, lokasi..."
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="acc_tracer" {{ request('status') === 'acc_tracer' ? 'selected' : '' }}>ACC Tracer
                        </option>
                        <option value="acc_cgp" {{ request('status') === 'acc_cgp' ? 'selected' : '' }}>ACC CGP</option>
                        <option value="revisi_tracer" {{ request('status') === 'revisi_tracer' ? 'selected' : '' }}>Revisi
                            Tracer</option>
                        <option value="revisi_cgp" {{ request('status') === 'revisi_cgp' ? 'selected' : '' }}>Revisi CGP
                        </option>
                    </select>
                </div>
            ...
            <div class="flex justify-end mt-4">
                <a href="{{ route('jalur.joint.index') }}"
                    class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded-md text-center">
                    Reset
                </a>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div>
                                        ...
                                        @if($joint->joint_line_optional && $joint->isEqualTee())
                                            <div class="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded inline-block">3-Way
                                                Connection</div>
                                        @endif
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500">
                                            {{ $joint->tipe_penyambungan ?: 'Tipe belum ditentukan' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="space-y-1">
                                        @if($joint->tracer_approved_at)
                                            <div class="flex items-center text-blue-600">
                                                ...
                                                <span class="text-xs">Tracer:
                                                    {{ $joint->tracer_approved_at->format('d/m/Y') }}</span>
                                            </div>
                                        @endif
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('jalur.joint.show', $joint) }}" onclick="savePageState()"
                                        ...
                                        @if(in_array($joint->status_laporan, ['draft', 'revisi_tracer', 'revisi_cgp']))
                                            <a href="{{ route('jalur.joint.edit', $joint) }}" onclick="savePageState()"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="text-gray-500">
                                        <svg class="mx-auto w-12 h-12 mb-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
