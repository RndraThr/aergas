  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      @if(in_array($lowering->status_laporan, ['draft', 'revisi_tracer', 'revisi_cgp']))
        <a href="{{ route('jalur.lowering.edit', $lowering) }}" class="px-4 py-2 bg-blue-100 text-blue-700 rounded hover:bg-blue-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($lowering->photoApprovals as $photo)
          <div class="border border-gray-200 rounded-lg overflow-hidden">
            ...
            <div class="aspect-w-4 aspect-h-3 bg-gray-100">
              @if($photo->photo_url && !empty(trim($photo->photo_url)))
                @php
                  ...
                  // Legacy internal paths - mark as unavailable for direct URL display
                  elseif (str_starts_with($imageUrl, 'JALUR_LOWERING/') || str_starts_with($imageUrl, 'aergas/')) {
                      $imageUrl = null; // Don't try to show these as direct URLs
                  }
                @endphp
                ...
                @else
                  ...
                  @if($isInternalPath)
                    ...
                    <div class="w-full h-48 bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center justify-center p-4">
            ...
            <!-- Photo Info -->
            <div class="p-4">
              <h3 class="font-medium text-gray-900 mb-2">{{ str_replace('foto_evidence_', '', $photo->photo_field_name) }}</h3>
  ...
  <div class="bg-white rounded-xl card-shadow p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="text-center">
        ...
        <div class="text-2xl font-bold text-gray-900">{{ number_format($lowering->lineNumber->estimasi_panjang, 1) }}</div>
      ...
      <div class="text-center">
        ...
        <div class="text-2xl font-bold text-blue-600">{{ number_format($lowering->lineNumber->total_penggelaran, 1) }}</div>
        <div class="text-sm text-blue-700">meter</div>
      </div>
      <div class="text-center">
        <div class="text-xs text-gray-500 uppercase tracking-wider mb-2">Progress</div>
        <div class="text-2xl font-bold text-green-600">{{ number_format($lowering->lineNumber->getProgressPercentage(), 1) }}%</div>
