<div class="flex">
    ...
    <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform lg:translate-x-0 lg:static lg:inset-0 transition-transform duration-300 ease-in-out border-r border-gray-200 flex flex-col"
        ...
        <div x-data="{ logos: [
            ...
            ], current: 0 }" x-init="setInterval(() => { current = (current + 1) % logos.length }, 3000)"
            ...
            <div class="flex items-center justify-center flex-1">
                <template x-for="(logo, index) in logos" :key="index">
                    <img :src="logo" alt="Logo Carousel" class="h-12 w-auto transition-opacity duration-700"
        ...
        <div class="p-6 bg-gradient-to-br from-gray-50 to-white border-b border-gray-200 flex-shrink-0">
            <div class="flex items-center space-x-3">
                <div
                    class="w-12 h-12 bg-gradient-to-br from-aergas-navy to-aergas-orange rounded-full flex items-center justify-center text-white font-medium shadow-md">
            ...
            <div class="space-y-1">
                ...
                @if(auth()->user()->hasAnyRole(['admin', 'cgp', 'tracer', 'super_admin', 'jalur']))
                    ...
                    <div x-data="{ dashboardOpen: {{ $isDashboardActive ? 'true' : 'false' }} }" class="space-y-1">
                        <button @click="dashboardOpen = !dashboardOpen"
                            ...
                            <i
                                class="fas fa-chart-pie w-5 mr-3 text-lg {{ $isDashboardActive ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        ...
                        <!-- Submenu -->
                        <div x-show="dashboardOpen" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 transform -translate-y-1"
                            x-transition:enter-end="opacity-100 transform translate-y-0"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 transform translate-y-0"
                            x-transition:leave-end="opacity-0 transform -translate-y-1" class="space-y-1 ml-6 mt-1">
                ...
                @if(auth()->user()->hasAnyRole(['admin', 'sk', 'sr', 'gas_in', 'cgp', 'tracer', 'super_admin']))
                    <a href="{{ route('customers.index') }}"
                        ...
                        <i
                            class="fas fa-users w-5 mr-3 text-lg {{ request()->routeIs('customers.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        Data Calon Pelanggan
                ...
                @if(auth()->user()->hasAnyRole(['sk', 'admin', 'super_admin']))
                    ...
                    <a href="{{ route('sk.index') }}"
                        ...
                        <i
                            class="fas fa-fire w-5 mr-3 text-lg {{ request()->routeIs('sk.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        SK Form
                ...
                @if(auth()->user()->hasAnyRole(['sr', 'admin', 'super_admin']))
                    ...
                    <a href="{{ route('sr.index') }}"
                        ...
                        <i
                            class="fas fa-house-user w-5 mr-3 text-lg {{ request()->routeIs('sr.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        SR Form
                ...
                @if(auth()->user()->hasAnyRole(['gas_in', 'admin', 'super_admin']))
                    ...
                    <a href="{{ route('gas-in.index') }}"
                        ...
                        <i
                            class="fas fa-gas-pump w-5 mr-3 text-lg {{ request()->routeIs('gas-in.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        Gas In Form
                ...
                @if(auth()->user()->hasAnyRole(['hse', 'admin', 'super_admin']))
                    ...
                    <a href="{{ route('hse.daily-reports.index') }}"
                        ...
                        <i
                            class="fas fa-hard-hat w-5 mr-3 text-lg {{ request()->routeIs('hse.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        HSE Daily Reports
                ...
                @if(auth()->user()->hasAnyRole(['admin', 'super_admin']))
                    ...
                    <div x-data="{ jalurOpen: {{ $isJalurActive ? 'true' : 'false' }} }" class="space-y-1">
                        <button @click="jalurOpen = !jalurOpen"
                            ...
                            <i
                                class="fas fa-road w-5 mr-3 text-lg {{ $isJalurActive ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        ...
                        <!-- Submenu -->
                        <div x-show="jalurOpen" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 transform -translate-y-1"
                            x-transition:enter-end="opacity-100 transform translate-y-0"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 transform translate-y-0"
                            ...
                            <a href="{{ route('jalur.test-package.index') }}"
                                class="flex items-center px-3 py-2 text-sm rounded-lg transition-all duration-200 {{ request()->routeIs('jalur.test-package.*') ? 'text-aergas-orange bg-aergas-orange/10' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}">
                                <i class="fas fa-vial w-5 mr-3 text-sm"></i>
                                Test Packages
                            </a>
                        </div>
                    </div>
                ...
                @elseif (auth()->user()->hasAnyRole(['jalur']))
                    ...
                    <div x-data="{ jalurOpen: {{ $isJalurActiveForJalurRole ? 'true' : 'false' }} }" class="space-y-1">
                        <button @click="jalurOpen = !jalurOpen"
                            ...
                            <i
                                class="fas fa-road w-5 mr-3 text-lg {{ $isJalurActiveForJalurRole ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                            <span class="flex-1 text-left">Jalur Management</span>
                            <svg class="w-5 h-5 transition-transform duration-200" :class="{ 'rotate-180': jalurOpen }"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7">
                    ...
                    <a href="{{ route('jalur.reports') }}"
                        ...
                        <i
                            class="fas fa-chart-bar w-5 mr-3 text-lg {{ request()->routeIs('jalur.reports*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        Laporan Jalur
                ...
                {{-- @if(auth()->user()->hasAnyRole(['admin']) && !auth()->user()->hasAnyRole(['tracer',
                'super_admin']))
                ...
                <a href="{{ route('approvals.tracer.index') }}"
                    ...
                    <i
                        class="fas fa-search mr-3 text-lg {{ request()->routeIs('approvals.tracer.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                    Photo Review
                    @php
                    $pendingTracer = \App\Models\PhotoApproval::where('photo_status', 'tracer_pending')->count();
                    @endphp
                    @if($pendingTracer > 0)
                    <span class="ml-auto bg-blue-500 text-white text-xs px-2 py-1 rounded-full">{{ $pendingTracer
                        }}</span>
                    @endif
                </a>
                @endif
                ...
                <a href="{{ route('approvals.cgp.index') }}"
                    ...
                    <i
                        class="fas fa-clipboard-check mr-3 text-lg {{ request()->routeIs('approvals.cgp.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                    Photo Review
                    @php
                    $pendingCgp = \App\Models\PhotoApproval::where('photo_status', 'cgp_pending')->count();
                    @endphp
                    @if($pendingCgp > 0)
                    <span class="ml-auto bg-green-500 text-white text-xs px-2 py-1 rounded-full">{{ $pendingCgp
                        }}</span>
                    @endif
                </a>
                @endif --}}
                ...
                @if(auth()->user()->hasAnyRole(['tracer', 'admin', 'super_admin']))
                    <div x-data="{ tracerOpen: {{ request()->routeIs('approvals.tracer.*') ? 'true' : 'false' }} }"
                        class="space-y-1">
                        ...
                        <!-- Submenu -->
                        <div x-show="tracerOpen" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 transform -translate-y-1"
                            x-transition:enter-end="opacity-100 transform translate-y-0"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 transform translate-y-0"
                ...
                @if(auth()->user()->hasAnyRole(['cgp', 'super_admin']))
                    <div x-data="{ cgpOpen: {{ request()->routeIs('approvals.cgp.*') ? 'true' : 'false' }} }"
                        class="space-y-1">
                        ...
                        <!-- Submenu -->
                        <div x-show="cgpOpen" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 transform -translate-y-1"
                            x-transition:enter-end="opacity-100 transform translate-y-0"
                            x-transition:leave="transition ease-in duration-150"
                            x-transition:leave-start="opacity-100 transform translate-y-0"
                ...
                @if(auth()->user()->hasAnyRole(['admin', 'super_admin']))
                    ...
                    <a href="{{ route('notifications.index') }}"
                        ...
                        <i
                            class="fas fa-bell w-5 mr-3 text-lg {{ request()->routeIs('notifications.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        Notifications
                    </a>

                    <!-- Reports with Submenu -->
                    <div class="relative"
                        x-data="{ open: {{ request()->routeIs('reports.*') || request()->routeIs('jalur.reports*') ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            ...
                            <i
                                class="fas fa-chart-bar w-5 mr-3 text-lg {{ request()->routeIs('reports.*') || request()->routeIs('jalur.reports*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                            Reports
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-200"
                            ...
                            class="mt-1 space-y-1 ml-6 transform-origin-top">
                ...
                @if(auth()->user()->hasAnyRole(['admin', 'super_admin']))
                    ...
                    <div x-data="{ open: {{ request()->routeIs('imports.*') ? 'true' : 'false' }} }" class="relative">
                        <button @click="open = !open"
                            ...
                            <i
                                class="fas fa-file-excel w-5 mr-3 text-lg {{ request()->routeIs('imports.*') ? 'text-aergas-orange' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                            Import Excel
                ...
                @if(auth()->user()->hasAnyRole(['super_admin', 'admin']))
                    ...
                    <a href="{{ route('admin.settings') }}"
                        ...
                        <i
                            class="fas fa-cogs w-5 mr-3 text-lg {{ request()->routeIs('admin.settings') ? 'text-white' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        System Settings
                    </a>
