...
@section('title', 'Import Koordinat Pelanggan')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">
  <h1 class="text-2xl font-bold">Import Koordinat Pelanggan</h1>

  <div class="flex items-center gap-3">
    <a href="{{ route('imports.coordinates.template') }}" class="px-3 py-2 bg-gray-100 rounded">Download Template</a>
  </div>

  <form method="POST" action="{{ route('imports.coordinates.import') }}" enctype="multipart/form-data" class="space-y-4">
    @csrf
    <div>
  ...
  @if (session('coordinate_import_results'))
    @php($r = session('coordinate_import_results'))
    <div class="p-4 bg-green-50 rounded">
      <h2 class="font-semibold mb-2">Hasil Import Koordinat</h2>
      <div>Berhasil: <b>{{ $r['success'] }}</b></div>
      <div>Diperbarui: <b>{{ $r['updated'] }}</b></div>
      <div>Tidak ditemukan: <b>{{ $r['not_found'] }}</b></div>
      <div>Gagal: <b>{{ count($r['failed']) }}</b></div>
