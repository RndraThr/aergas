...
@section('content')
  <div class="space-y-6" x-data="gasInIndexData()" x-init="initPaginationState(); window.gasInData = $data">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      ...
      <div class="grid grid-cols-2 md:flex gap-2">
        ...
        <button @click="showExportModal = true"
          class="px-3 md:px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 text-sm md:text-base whitespace-nowrap">
        ...
        <button @click="showDownloadModal = true"
          class="px-3 md:px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm md:text-base whitespace-nowrap">
        ...
        {{-- Create Button --}}
        <a href="{{ route('gas-in.create') }}"
          class="px-3 md:px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm md:text-base whitespace-nowrap text-center">
            ...
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-sm">
                <template x-if="row.sr_data && row.sr_data.no_seri_mgrt">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      <button
                        @click="downloadFotoMGRT(row.reff_id_pelanggan, row.calon_pelanggan?.nama_pelanggan, row.tanggal_gas_in)"
                  ...
                  <template x-if="row.rejected_photos_count > 0">
                    <div class="relative">
                      <button class="text-red-600 hover:text-red-800 text-xs flex items-center gap-1"
                        :data-gasin-id="row.id" @mouseenter="showRejectionPopup(row.id, $event.target)"
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex justify-end gap-1">
                  <a :href="`/gas-in/${row.id}`" @click="savePageState()"
                  ...
                  <template x-if="canEdit(row)">
                    <a :href="`/gas-in/${row.id}/edit`" @click="savePageState()"
                      class="px-3 py-1.5 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200" title="Edit Gas In">
                      <i class="fas fa-edit mr-1"></i>Edit
                    </a>
                  </template>
                  <template x-if="canEdit(row)">
                    <button @click="confirmDelete(row.id, row.reff_id_pelanggan)"
                      class="px-3 py-1.5 text-xs bg-red-100 text-red-700 rounded hover:bg-red-200" title="Hapus Gas In">
    ...
    {{-- Download Modal --}}
    <div x-show="showDownloadModal" x-cloak @click.self="closeDownloadModal()"
      class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" style="display: none;">
      <div @click.stop class="bg-white rounded-xl p-6 w-full mx-4 shadow-2xl"
        :class="downloadStep === 'preview' ? 'max-w-3xl' : 'max-w-md'">
          ...
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600"
                x-text="downloadStep === 'filter' ? 'Pilih Rentang Tanggal' : 'Preview Data'"></p>
            </div>
          </div>
        ...
        <div x-show="downloadStep === 'filter'" class="mb-6">
          ...
          <div class="space-y-3">
            <div>
              ...
              <input type="date" x-model="downloadFilters.tanggal_dari"
            ...
            <div>
              ...
              <input type="date" x-model="downloadFilters.tanggal_sampai"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-green-500">
            </div>
          </div>
        ...
        <div x-show="downloadStep === 'preview'" class="mb-6">
          <div class="mb-4 flex items-center justify-between">
            <div>
              <p class="font-semibold text-gray-800">
                Total File:
                <span :class="previewData.length > 100 ? 'text-orange-600' : 'text-green-600'"
                  x-text="previewData.length"></span>
              </p>
          ...
          <div x-show="previewData.length > 100" class="mb-3 p-3 bg-orange-50 border border-orange-200 rounded">
            <p class="text-xs text-orange-700">
              ...
              <strong>Perhatian:</strong> Download dalam jumlah besar (<span x-text="previewData.length"></span> file)
              dapat memakan waktu lama (3-5 menit) dan berisiko timeout. Mohon tunggu hingga proses selesai.
            </p>
          </div>
        ...
        <div class="flex gap-3">
          <button @click="closeDownloadModal()" :disabled="downloadLoading"
          ...
          <button x-show="downloadStep === 'filter'" @click="loadPreview()" :disabled="previewLoading"
          ...
          <button x-show="downloadStep === 'preview'" @click="executeDownload()" :disabled="downloadLoading"
    ...
    <div x-show="downloadLoading" x-cloak
      class="fixed inset-0 bg-black bg-opacity-75 z-[60] flex items-center justify-center" style="display: none;">
        ...
        <div class="mb-6">
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <div class="flex items-start text-left text-xs text-blue-700 space-y-2">
              ...
              <div>
                <p class="mb-2">
                  <i class="fas fa-check text-green-600 mr-1"></i> Mengunduh <span class="font-semibold"
                    x-text="previewData.length"></span> foto dari Google Drive
                </p>
                <p class="mb-2">
                ...
                <p>
                  ...
                  <span
                    x-text="previewData.length > 100 ? '3-5 menit' : previewData.length > 50 ? '1-3 menit' : '< 1 menit'"></span>
                </p>
              </div>
            </div>
          </div>
    ...
    {{-- Export Excel Modal --}}
    <div x-show="showExportModal" x-cloak @click.self="closeExportModal()"
      class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" style="display: none;">
      <div @click.stop class="bg-white rounded-xl p-6 w-full mx-4 shadow-2xl"
        :class="exportStep === 'preview' ? 'max-w-4xl' : 'max-w-md'">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600"
                x-text="exportStep === 'filter' ? 'Pilih Filter Data' : 'Preview Data Export'"></p>
            </div>
          </div>
        ...
        <div x-show="exportStep === 'filter'" class="mb-6">
          ...
          <div class="space-y-3">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Gas In (Dari)</label>
              <input type="date" x-model="exportFilters.tanggal_dari"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-emerald-500">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Gas In (Sampai)</label>
              <input type="date" x-model="exportFilters.tanggal_sampai"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-emerald-500">
            </div>
            <div>
        ...
        <div class="flex gap-3">
          ...
          <button x-show="exportStep === 'filter'" @click="loadExportPreview()" :disabled="exportPreviewLoading"
            class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
          ...
          <button x-show="exportStep === 'preview'" @click="executeExport()"
  ...
  <div id="rejection-popup-container"
    ...
    onmouseenter="keepPopupOpen(window.currentGasInId)" onmouseleave="hideRejectionPopup(window.currentGasInId)">
  ...
  @push('scripts')
    <script>
      ...
      function showRejectionPopup(gasInId, triggerElement) {
        ...
        loadedRejections.delete(gasInId);
      ...
      async function loadRejectionPopup(gasInId) {
        ...
        try {
          ...
          if (window.currentGasInId !== gasInId) return;
