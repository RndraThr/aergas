...
<div class="space-y-6">
  ...
  <form method="get" action="{{ route('gas-in.index') }}" class="bg-white p-4 rounded-xl card-shadow">
    <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
      ...
      <div>
        <select name="module_status" class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-green-500">
...
<div id="successToast" class="fixed top-4 right-4 bg-green-600 text-white px-6 py-4 rounded-lg shadow-2xl transform translate-x-full transition-transform duration-300 z-50">
  <div class="flex items-center">
    ...
    <div>
      ...
      <p class="text-sm text-green-100" id="deleteDetails">Gas In telah dihapus dari sistem</p>
    </div>
  </div>
</div>
...
async function executeDelete() {
  ...
  try {
    ...
    const response = await fetch(`/gas-in/${deleteId}`, {
      method: 'DELETE',
      headers: {
...
function showSuccessToast(result = {}) {
  ...
  let message = 'Gas In telah dihapus dari sistem';
