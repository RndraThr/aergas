  ...
  <div class="space-y-6" x-data="skEdit()" x-init="init()">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <a href="{{ route('sk.show', $sk->id) }}"
          class="px-4 py-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">Detail</a>
    ...
    <div class="bg-white rounded-xl card-shadow p-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <select x-model="createdBy"
            class="mt-1 w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 text-sm">
    ...
    <form @submit.prevent="updateMaterial" class="bg-white rounded-xl card-shadow p-6 space-y-4">
      ...
      <div class="flex justify-end gap-3 pt-4">
        <button type="submit" :disabled="updating || !isMaterialComplete()"
        ...
        @foreach ($photoDefs as $ph)
          <div class="border rounded-lg p-4" x-data="slotUploader('{{ $ph['field'] }}')">
            ...
            <div class="flex items-center gap-2 mt-3">
              <input class="hidden" type="file" :id="`file_${@js($ph['field'])}`"
                accept="{{ implode(',', (array) $ph['accept']) }}" @change="onPick($event)">
              <label :for="`file_${@js($ph['field'])}`"
