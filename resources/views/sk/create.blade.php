...
@section('content')
  ...
  <div class="space-y-6" x-data="skCreate()" x-init="init()">
    ...
    <div class="flex items-center justify-between">
      ...
      <a href="javascript:void(0)" onclick="goBackWithPagination('{{ route('sk.index') }}')"
        class="px-4 py-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">Kembali</a>
    </div>
    ...
    <form class="bg-white rounded-xl card-shadow p-6 space-y-8" @submit.prevent="onSubmit">
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div class="md:col-span-10">
            <input type="text" x-model="reff" placeholder="Ketik Reference ID…"
              ...
              <div class="bg-red-50 border border-red-200 p-4 rounded">
                <div class="flex items-start">
                  ...
                  <div>
                    ...
                    <p class="text-red-700 text-sm mt-1">Customer ini memiliki status "batal" dan tidak dapat melanjutkan
                      ke modul SK. Silakan hubungi admin untuk informasi lebih lanjut.</p>
                  </div>
                </div>
              </div>
            </template>
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Instalasi <span
                class="text-red-500">*</span></label>
            <input type="date" x-model="tanggal" class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500"
              required>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="lg:col-span-3">
              ...
              <input type="number" x-model="material.panjang_pipa_gl_medium_m" step="0.01" min="0" max="1000"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500" placeholder="0.00" required>
            </div>
            ...
            <div>
              ...
              <input type="number" x-model="material.qty_elbow_1_2_galvanis" min="0" max="100"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500" placeholder="0" required>
            </div>
            ...
            <div>
              ...
              <input type="number" x-model="material.qty_tee_1_2" min="0" max="100"
                class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500" placeholder="0">
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <template x-for="(ph, i) in photoDefs" :key="ph.field">
            <div
              ...
              <!-- Hidden file input -->
              <input type="file" :id="`inp_${ph.field}`" :accept="acceptString(ph.accept)" class="hidden"
                :disabled="!customer || !reff" @change="handleFileSelect($event, ph.field)">
              ...
              <template x-if="!previews[ph.field]">
                <div class="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p class="font-medium" :class="dragStates[ph.field] ? 'text-blue-600' : 'text-gray-600'">
                    <span x-show="!dragStates[ph.field]">Klik untuk upload</span>
                    <span x-show="dragStates[ph.field]">Lepaskan file di sini</span>
                  </p>
                  <p class="text-xs mt-1" :class="dragStates[ph.field] ? 'text-blue-400' : 'text-gray-400'">
      ...
      return {
        ...
        async onSubmit() {
          ...
          if (this.customer && this.customer.status === 'batal') {
            alert('Customer dengan status "batal" tidak dapat melanjutkan ke modul SK.');
            return;
          }
