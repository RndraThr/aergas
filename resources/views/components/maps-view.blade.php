...
<div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100" x-data="mapsComponent()"
    x-init="initMapWithDrawingTools()">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-2xl font-bold text-white" x-text="mapsStats.total_customers || 0">0
                                </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-2xl font-bold text-white"
                                    x-text="mapsStats.customers_with_coordinates || 0">0</div>
                                <div class="text-white/80 text-sm">Dengan Koordinat</div>
                                <div class="text-xs text-white/60"
                                    x-text="(mapsStats.coordinate_coverage_rate || 0) + '% coverage'">0% coverage</div>
                            </div>
                    ...
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-2xl font-bold text-white"
                                    x-text="mapsStats.customers_without_coordinates || 0">0</div>
                    ...
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="text-xs text-white/60"
                                    x-text="(mapsStats.completion_rate || 0) + '% completion rate'">0% completion rate
                                </div>
                            </div>
                    ...
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-2xl font-bold text-white" x-text="mapsStats.pending_review || 0">0
                                </div>
                    ...
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-2xl font-bold text-white" x-text="mapsStats.photo_approved_count || 0">
                                    0</div>
                                <div class="text-white/80 text-sm">Photo Approved</div>
                                <div class="text-xs text-white/60" x-text="(mapsStats.photo_approval_rate || 0) + '%'">
                                    0%</div>
                    ...
                    <template x-for="customer in searchResults" :key="customer.id">
                        <div @click="selectCustomer(customer)"
                            ...
                            <div class="font-medium text-gray-900"
                                x-text="customer.reff_id + ' - ' + (customer.title || 'N/A')"></div>
        ...
        <div x-show="hasActiveFilters()" class="mt-4">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <template x-for="filter in getActiveFiltersDisplay()" :key="filter.key">
                    <span
                        class="inline-flex items-center gap-1 px-3 py-1 bg-aergas-orange text-white text-sm rounded-full">
    ...
    <div class="relative">
        ...
        <div class="absolute bottom-4 left-4 z-[1000]" x-data="{ expanded: false, activeTab: 'layers' }">
                ...
                <div class="max-h-96 overflow-y-auto">
                    ...
                    <div x-show="activeTab === 'shapes'" class="p-3">
                        ...
                        <div class="flex gap-2 mb-3">
                            <button @click="showAllShapes()"
                                class="flex-1 px-2 py-1 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200 transition-colors">
                                <i class="fas fa-eye mr-1"></i> Show All
                            </button>
                            <button @click="hideAllShapes()"
                                class="flex-1 px-2 py-1 bg-red-100 text-red-700 rounded text-xs hover:bg-red-200 transition-colors">
                        ...
                        <div class="space-y-1 max-h-64 overflow-y-auto">
                            ...
                            <template x-for="feature in mapFeatures" :key="feature.id">
                                <div
                                    class="flex items-center justify-between p-2 hover:bg-gray-50 rounded border border-gray-100 group">
                                    <div class="flex items-center gap-2 flex-1 min-w-0">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="text-xs text-gray-500"
                                                x-text="getShapeTypeLabel(feature.properties.feature_type)"></div>
                                        </div>
                                    </div>
        ...
        <div class="absolute top-4 left-32 bg-white rounded-lg shadow-xl border border-gray-200 p-3 max-h-[calc(100vh-8rem)] overflow-y-auto"
            ...
            <div x-show="activeTab === 'tools'" class="space-y-2 w-52">
                ...
                <div x-show="currentDrawingType === 'polygon' || currentDrawingType === 'circle'"
                ...
                <template x-for="feature in mapFeatures" :key="feature.id">
                    <div class="flex items-center justify-between p-1 bg-gray-50 rounded">
                        <div class="flex items-center space-x-1">
                            <input type="checkbox" :checked="feature.properties.is_visible"
                                @change="toggleFeatureVisibility(feature)" class="scale-75">
        ...
        <div class="map-legend absolute top-4 right-4 max-h-[calc(100vh-8rem)] overflow-y-auto"
            x-data="{ legendExpanded: false }">
            <!-- Collapsed Legend Button -->
            <div x-show="!legendExpanded"
                class="bg-white rounded-lg shadow-xl border border-gray-200 p-3 cursor-pointer hover:shadow-2xl transition-all"
                @click="legendExpanded = true">
            ...
            <div x-show="legendExpanded" x-transition:enter="transition ease-out duration-200"
                ...
                <!-- Legend Header -->
                <div class="bg-gradient-to-r from-aergas-navy to-aergas-orange p-3 rounded-t-lg cursor-pointer hover:shadow-lg transition-shadow"
                    @click="legendExpanded = false">
                ...
                <div class="p-3 space-y-1 overflow-y-auto flex-1">
                    ...
                    <div>
                        ...
                        <div class="text-xs">
                            <div class="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendStatus('pending')">
                                <div class="flex items-center">
                                    <div class="w-3 h-3 rounded-full bg-blue-500 mr-2 flex-shrink-0"></div>
                                    <span class="text-gray-700"
                                        :class="legendFilters.hiddenStatuses.includes('pending') ? 'line-through opacity-50' : ''">Pending</span>
                                </div>
                                <span class="text-blue-600 font-medium"
                                    x-text="(mapsStats.status_counts && mapsStats.status_counts.pending) || 0">0</span>
                            </div>
                            <div class="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendStatus('lanjut')">
                                <div class="flex items-center">
                                    <div class="w-3 h-3 rounded-full bg-green-500 mr-2 flex-shrink-0"></div>
                                    <span class="text-gray-700"
                                        :class="legendFilters.hiddenStatuses.includes('lanjut') ? 'line-through opacity-50' : ''">Lanjut</span>
                                </div>
                                <span class="text-green-600 font-medium"
                                    x-text="(mapsStats.status_counts && mapsStats.status_counts.lanjut) || 0">0</span>
                            </div>
                            <div class="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendStatus('in_progress')">
                                <div class="flex items-center">
                                    <div class="w-3 h-3 rounded-full bg-yellow-500 mr-2 flex-shrink-0"></div>
                                    <span class="text-gray-700"
                                        :class="legendFilters.hiddenStatuses.includes('in_progress') ? 'line-through opacity-50' : ''">In
                                        Progress</span>
                                </div>
                                <span class="text-yellow-600 font-medium"
                                    x-text="(mapsStats.status_counts && mapsStats.status_counts.in_progress) || 0">0</span>
                            </div>
                            <div class="flex items-center justify-between cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendStatus('batal')">
                                <div class="flex items-center">
                                    <div class="w-3 h-3 rounded-full bg-red-500 mr-2 flex-shrink-0"></div>
                                    <span class="text-gray-700"
                                        :class="legendFilters.hiddenStatuses.includes('batal') ? 'line-through opacity-50' : ''">Batal</span>
                                </div>
                    ...
                    <div class="pt-2 border-t border-gray-200">
                        ...
                        <div class="text-xs">
                            <div class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendProgress('validasi')">
                                <div
                                    class="w-5 h-5 bg-purple-500 rounded-full mr-2 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-user-check text-white text-[8px]"></i>
                                </div>
                                <span class="text-gray-700"
                                    :class="legendFilters.hiddenProgress.includes('validasi') ? 'line-through opacity-50' : ''">Validasi</span>
                            </div>
                            <div class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendProgress('sk')">
                                <div
                                    class="w-5 h-5 bg-orange-500 rounded-full mr-2 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-wrench text-white text-[8px]"></i>
                                </div>
                                <span class="text-gray-700"
                                    :class="legendFilters.hiddenProgress.includes('sk') ? 'line-through opacity-50' : ''">SK
                                    (Sambungan Kompor)</span>
                            </div>
                            <div class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendProgress('sr')">
                                <div
                                    class="w-5 h-5 bg-blue-500 rounded-full mr-2 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-home text-white text-[8px]"></i>
                                </div>
                                <span class="text-gray-700"
                                    :class="legendFilters.hiddenProgress.includes('sr') ? 'line-through opacity-50' : ''">SR
                                    (Sambungan Rumah)</span>
                            </div>
                            <div class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendProgress('gas_in')">
                                <div
                                    class="w-5 h-5 bg-red-500 rounded-full mr-2 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-fire text-white text-[8px]"></i>
                                </div>
                                <span class="text-gray-700"
                                    :class="legendFilters.hiddenProgress.includes('gas_in') ? 'line-through opacity-50' : ''">Gas
                                    In</span>
                            </div>
                            <div class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors"
                                @click="toggleLegendProgress('done')">
                                <div
                                    class="w-5 h-5 bg-green-500 rounded-full mr-2 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-check-circle text-white text-[8px]"></i>
                                </div>
                                <span class="text-gray-700"
                                    :class="legendFilters.hiddenProgress.includes('done') ? 'line-through opacity-50' : ''">Done</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-1 text-xs">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-800"
                                    x-text="(mapsStats.coordinate_coverage_rate || 0) + '%'">0%</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Completion:</span>
                                <span class="font-medium text-gray-800"
                                    x-text="(mapsStats.completion_rate || 0) + '%'">0%</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function mapsComponent() {
        return {
            ...
            async loadMapFeatures() {
                try {
                    const response = await fetch('/map-features?context=main', {
                        method: 'GET',
                        headers: {
