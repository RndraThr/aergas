    ...
    <div class="container mx-auto px-4 py-6" x-data="cgpCustomersData()" x-init="initPaginationState()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-white overflow-hidden shadow rounded-lg">
                ...
                <div class="bg-gray-50 px-5 py-3">
                    <div class="text-xs text-gray-500">
                        Total foto yang ditolak/perlu revisi.
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <button @click="resetFilters()"
                        class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-1 rounded-md text-sm">
                        Reset
                    </button>
                </div>
            </div>
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Customer Info</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CGP
                                Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Progress</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <template x-for="customer in customers" :key="customer.reff_id_pelanggan">
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="text-sm font-medium text-gray-900" x-text="customer.reff_id_pelanggan">
                                        </div>
                                        <div class="text-sm text-gray-600" x-text="customer.nama_pelanggan"></div>
                                        <div class="text-xs text-gray-500"
                                            x-text="customer.alamat?.substring(0, 50) + (customer.alamat?.length > 50 ? '...' : '')">
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sk_ready && !customer.cgp_status?.sk_completed">
                                            <span
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sk_in_progress && !customer.cgp_status?.sk_ready && !customer.cgp_status?.sk_completed">
                                            <span
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sk_waiting_tracer && !customer.cgp_status?.sk_ready && !customer.cgp_status?.sk_in_progress && !customer.cgp_status?.sk_completed">
                                            <span
                                        ...
                                        <template x-if="customer.cgp_status?.sk_rejections?.has_rejections">
                                            <div class="mt-1.5 space-y-1">
                                                <template x-for="rejection in customer.cgp_status.sk_rejections.all"
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="flex items-start gap-1.5">
                                                            ...
                                                            <div class="flex-1 min-w-0">
                                                                ...
                                                                <div class="text-red-600" x-show="rejection.notes"
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sr_ready && !customer.cgp_status?.sr_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                                ⏳ SR Ready for CGP
                                            </span>
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sr_in_progress && !customer.cgp_status?.sr_ready && !customer.cgp_status?.sr_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                🔄 SR In Progress
                                            </span>
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.sr_waiting_tracer && !customer.cgp_status?.sr_ready && !customer.cgp_status?.sr_in_progress && !customer.cgp_status?.sr_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                                ⏱️ SR Waiting Tracer
                                            </span>
                                        ...
                                        <template x-if="customer.cgp_status?.sr_rejections?.has_rejections">
                                            <div class="mt-1.5 space-y-1">
                                                <template x-for="rejection in customer.cgp_status.sr_rejections.all"
                                                    :key="rejection.field_name">
                                                    <div
                                                        class="bg-red-50 border border-red-200 rounded px-2 py-1.5 text-xs">
                                                        <div class="flex items-start gap-1.5">
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.gas_in_ready && !customer.cgp_status?.gas_in_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.gas_in_in_progress && !customer.cgp_status?.gas_in_ready && !customer.cgp_status?.gas_in_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                🔄 Gas In In Progress
                                            </span>
                                        ...
                                        <template
                                            x-if="customer.cgp_status?.gas_in_waiting_tracer && !customer.cgp_status?.gas_in_ready && !customer.cgp_status?.gas_in_in_progress && !customer.cgp_status?.gas_in_completed">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                        ...
                                        <template x-if="customer.cgp_status?.gas_in_rejections?.has_rejections">
                                            <div class="mt-1.5 space-y-1">
                                                <template x-for="rejection in customer.cgp_status.gas_in_rejections.all"
                                                    :key="rejection.field_name">
                                                    <div
                                                        class="bg-red-50 border border-red-200 rounded px-2 py-1.5 text-xs">
                                                        <div class="flex items-start gap-1.5">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-1">
                                        ...
                                        <div class="w-4 h-px"
                                            :class="customer.cgp_status?.sk_completed ? 'bg-green-400' : 'bg-gray-200'">
                                        </div>
                                        ...
                                        <div class="w-4 h-px"
                                            :class="customer.cgp_status?.sr_completed ? 'bg-green-400' : 'bg-gray-200'">
                                        </div>
                        ...
                        <tr x-show="customers.length === 0">
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">Belum ada pelanggan yang ready untuk CGP review
                                    </p>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div class="bg-red-50 border-b border-red-200 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900" x-text="moduleTitle + ' Rejections'">
                                    </h3>
                                    <p class="text-sm text-gray-600" x-text="`${rejectionData.count} photo(s) rejected`">
                                    </p>
                                </div>
                            </div>
                        ...
                        <template x-for="userRejections in rejectionData.by_user" :key="userRejections.user_id">
                            <div class="mb-6 last:mb-0">
                                ...
                                <div class="flex items-center mb-3 pb-2 border-b border-gray-200">
                                    <div
                                        class="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center mr-2">
                                        <span class="text-white text-xs font-medium"
                                            x-text="userRejections.user_name.charAt(0).toUpperCase()"></span>
                                    </div>
                                    <div>
                                        <div class="font-semibold text-gray-900" x-text="userRejections.user_name"></div>
                                        <div class="text-xs text-gray-500"
                                            x-text="`Rejected ${userRejections.count} photo(s)`"></div>
                                    </div>
                                </div>
                                    ...
                                    <template x-for="photo in userRejections.photos" :key="photo.field_name">
                                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                            ...
                                            <div class="mt-2" x-show="photo.notes">
                                                ...
                                                <div class="text-sm text-gray-700 bg-white rounded p-2 border border-gray-200"
                                                    x-text="photo.notes || '-'"></div>
                                            </div>
                                        </div>
                                    </template>
                                </div>
