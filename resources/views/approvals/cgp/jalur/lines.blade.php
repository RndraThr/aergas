...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-green-50 to-emerald-50" x-data="linesData()" x-init="init()">
    <div class="container-fluid px-4 py-8">
        {{-- Breadcrumb --}}
        <div class="mb-6">
            <a href="{{ route('approvals.cgp.jalur.clusters') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white text-gray-700 rounded-lg hover:bg-gray-50 transition-all shadow-sm hover:shadow-md group">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div class="grid grid-cols-2 md:grid-cols-6 gap-3">
                    <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-3 border border-blue-200">
                        ...
                        <div class="text-xl font-bold text-blue-900" x-text="stats.total_items || 0"></div>
                    </div>
                    <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg p-3 border border-indigo-200">
                        <div class="text-xs font-medium text-indigo-600 mb-1">Lines</div>
                        <div class="text-xl font-bold text-indigo-900" x-text="stats.total_lines || 0"></div>
                    </div>
                    <div class="bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-lg p-3 border border-cyan-200">
                        <div class="text-xs font-medium text-cyan-600 mb-1">Joints</div>
                        <div class="text-xl font-bold text-cyan-900" x-text="stats.total_joints || 0"></div>
                    </div>
                    <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-3 border border-orange-200">
                        ...
                        <div class="text-xl font-bold text-orange-900" x-text="stats.pending_items || 0"></div>
                    </div>
                    <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-3 border border-green-200">
                        ...
                        <div class="text-xl font-bold text-green-900" x-text="stats.approved_items || 0"></div>
                    </div>
                    <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-lg p-3 border border-red-200">
                        ...
                        <div class="text-xl font-bold text-red-900" x-text="stats.rejected_items || 0"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6">
            <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
                {{-- Header --}}
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-6 py-3 border-b border-gray-200">
                ...
                <div class="p-6">
                    ...
                    <div x-show="filters.search || filters.filter !== 'all' || filters.date_from || filters.date_to" class="mt-4 pt-4 border-t border-gray-200">
                        <div class="flex items-center gap-2 flex-wrap">
                            <span class="text-xs font-medium text-gray-500">Filter Aktif:</span>

                            <span x-show="filters.search" class="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-medium">
                            ...
                            <span x-show="filters.filter !== 'all'" class="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-100 text-purple-700 rounded-md text-xs font-medium">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                </svg>
                                <span x-text="'Status: ' + (filters.filter === 'pending' ? 'Pending' : filters.filter === 'approved' ? 'Approved' : filters.filter === 'rejected' ? 'Rejected' : 'No Evidence')"></span>
                            </span>

                            <span x-show="filters.date_from" class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-100 text-green-700 rounded-md text-xs font-medium">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span x-show="filters.date_to" class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-100 text-green-700 rounded-md text-xs font-medium">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                </svg>
            ...
            <template x-for="item in items" :key="`${item.item_type}-${item.id}`">
                <div class="h-full">
                    ...
                    <template x-if="item.item_type === 'line'">
                        <div class="group bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col h-full">
                            <a :href="`/approvals/cgp/jalur/lines/${item.id}/evidence`" class="flex flex-col flex-1 h-full">
                        {{-- Card Header - Compact --}}
                        <div class="bg-gradient-to-r from-indigo-50 to-purple-50 px-5 py-3 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition" x-text="item.line_number"></h3>
                                        <span class="text-xs font-medium text-gray-600">
                                            <span x-text="item.approval_status_label"></span> (<span x-text="Math.round(item.approval_progress || 0)"></span>%)
                                        </span>
                                    </div>
                                </div>

                                {{-- Status Badge --}}
                                <span x-show="item.approval_stats.status === 'approved'" class="px-3 py-1 text-xs font-bold rounded-full bg-green-100 text-green-800 border border-green-300">
                                    ✓ Approved
                                </span>
                                <span x-show="item.approval_stats.status === 'rejected'" class="px-3 py-1 text-xs font-bold rounded-full bg-red-100 text-red-800 border border-red-300">
                                    ✗ Rejected
                                </span>
                                <span x-show="item.approval_stats.status === 'pending'" class="px-3 py-1 text-xs font-bold rounded-full bg-orange-100 text-orange-800 border border-orange-300">
                                    ⏳ Pending
                                </span>
                                <span x-show="!item.approval_stats.status || item.approval_stats.status === 'no_data'" class="px-3 py-1 text-xs font-bold rounded-full bg-gray-100 text-gray-600">
                                    No Data
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <div class="mb-3 p-3 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200">
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div>
                                        ...
                                        <span class="font-semibold text-gray-900">Ø<span x-text="item.diameter"></span>mm</span>
                                    </div>
                                    <div>
                                        <span class="text-gray-500">Jalan:</span>
                                        <span class="font-semibold text-gray-900" x-text="item.nama_jalan || '-'"></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 gap-2 mb-3">
                                <div class="text-center bg-blue-50 rounded-lg p-2 border border-blue-100">
                                    ...
                                    <div class="text-sm font-bold text-blue-900" x-text="Number(item.estimasi_panjang || 0).toFixed(1) + 'm'"></div>
                                </div>
                                <div class="text-center bg-indigo-50 rounded-lg p-2 border border-indigo-100">
                                    ...
                                    <div class="text-sm font-bold text-indigo-900" x-text="Number(item.total_penggelaran || 0).toFixed(1) + 'm'"></div>
                                </div>
                                <div class="text-center bg-green-50 rounded-lg p-2 border border-green-100">
                                    ...
                                    <div class="text-sm font-bold"
                                         ...
                                         x-text="item.actual_mc100 ? Number(item.actual_mc100).toFixed(1) + 'm' : '-'"></div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 flex-1 flex flex-col">
                                <div class="text-xs text-gray-600 mb-2 font-semibold">
                                    <span x-text="item.approval_stats.work_dates_count || 0"></span> Tanggal Pekerjaan
                                </div>
                                <div class="flex-1 max-h-32 overflow-y-auto space-y-1" x-show="item.approval_stats.work_dates_detail && item.approval_stats.work_dates_detail.length > 0">
                                    <template x-for="dateDetail in (item.approval_stats.work_dates_detail || [])" :key="dateDetail.date">
                            ...
                            {{-- Rejection Info - Tampilkan semua langsung --}}
                            <template x-if="item.approval_stats.rejections && item.approval_stats.rejections.has_rejections">
                                <div class="mb-3 space-y-1">
                                    <template x-for="rejection in item.approval_stats.rejections.all" :key="rejection.field_name">
                                        <div class="bg-red-50 border border-red-200 rounded px-2 py-1.5 text-xs">
                                            <div class="flex items-start gap-1.5">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <div class="text-red-600" x-show="rejection.notes && rejection.notes !== '-'" x-text="rejection.notes"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
                                    ...
                                    <div>
                                        <div class="flex items-center justify-between text-xs mb-2">
                                            <span class="text-gray-600">
                                                Photos: <span class="font-semibold text-gray-900"><span x-text="item.approval_stats.approved_photos"></span>/<span x-text="item.approval_stats.total_photos"></span></span>
                                            </span>
                                            <span class="font-bold text-gray-900" x-text="Math.round(item.approval_stats.percentage || 0) + '%'"></span>
                                        </div>
                    ...
                    <template x-if="item.item_type === 'joint'">
                        <div class="group bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border-2 border-blue-200 overflow-hidden flex flex-col h-full">
                            <a :href="`/approvals/cgp/jalur/joints/${item.id}/evidence`" class="flex flex-col flex-1 h-full">
                                {{-- Card Header - Compact --}}
                                <div class="bg-gradient-to-r from-blue-50 to-cyan-50 px-5 py-3 border-b border-blue-200">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h3 class="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition" x-text="item.nomor_joint"></h3>
                                                <span class="text-xs font-medium text-gray-600">
                                                    <span x-text="item.approval_status_label"></span> (<span x-text="Math.round(item.approval_progress || 0)"></span>%)
                                                </span>
                                            </div>
                                        ...
                                        <span x-show="!item.approval_stats.status || item.approval_stats.status === 'no_evidence'" class="px-3 py-1 text-xs font-bold rounded-full bg-gray-100 text-gray-600">
                                            No Data
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 flex-1 flex flex-col">
                                    ...
                                    <div class="mb-3 p-3 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg border border-blue-200">
                                        ...
                                        <div class="text-center mt-2 text-xs text-gray-600" x-show="item.joint_line_optional">
                                    ...
                                    <div class="mb-3 p-3 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200">
                                        <div class="grid grid-cols-2 gap-2 text-xs">
                                            <div>
                                                ...
                                                <span class="font-semibold text-gray-900" x-text="item.tipe_penyambungan || '-'"></span>
                                            </div>
                                            <div>
                                                <span class="text-gray-500">Tipe Fitting:</span>
                                                <span class="font-semibold text-gray-900" x-text="item.fitting_type?.nama_fitting || '-'"></span>
                                            </div>
                                            <div class="col-span-2" x-show="item.jalan">
                                                <span class="text-gray-500">Jalan:</span>
                                                <span class="font-semibold text-gray-900" x-text="item.jalan || '-'"></span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3 flex-1 flex flex-col">
                                        ...
                                        <div class="flex-1 max-h-32 overflow-y-auto space-y-1" x-show="item.approval_stats.work_dates_detail && item.approval_stats.work_dates_detail.length > 0">
                                            <template x-for="dateDetail in (item.approval_stats.work_dates_detail || [])" :key="dateDetail.date">
                                                <div class="flex items-center justify-between px-2 py-1 bg-blue-50 rounded border border-blue-200">
                                                    <span class="text-xs font-medium text-blue-800" x-text="new Date(dateDetail.date).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'})"></span>
                                                    <span class="text-xs font-bold text-blue-900" x-text="dateDetail.photo_count + ' foto'"></span>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-auto">
                                        ...
                                        <template x-if="!item.approval_stats.total_photos || item.approval_stats.total_photos === 0">
                                            <div class="text-center text-xs text-gray-400 py-2">
                                                Belum ada evidence
                                            </div>
                                        </template>
                                ...
                                {{-- Card Footer - Always at bottom --}}
                                <div class="px-4 py-2 bg-blue-50 border-t border-blue-200 flex items-center justify-between mt-auto">
            ...
            <div x-show="items.length === 0" class="col-span-2">
                <div class="bg-white rounded-xl shadow-md p-12 text-center border border-gray-100">
                    ...
                    <p class="text-gray-500">
                        <span x-show="filters.search">Tidak ditemukan line atau joint dengan kata kunci "<span x-text="filters.search"></span>"</span>
                        <span x-show="!filters.search && filters.filter !== 'all'">Tidak ada item dengan filter yang dipilih</span>
                        <span x-show="!filters.search && filters.filter === 'all'">Belum ada line atau joint dalam cluster ini</span>
                    </p>
                </div>
            </div>
        </div>
...
function linesData() {
    return {
        ...
        lines: @json($items->items() ?? []), // Keep for backward compatibility with template
        pagination: {
        ...
        async fetchLines(resetPage = false) {
            ...
            try {
                ...
                const response = await fetch(`/approvals/cgp/jalur/clusters/${this.clusterId}/lines?${params}`, {
                    headers: {
                ...
                if (data.success) {
                    ...
                    this.lines = data.data.data || []; // Keep for backward compatibility
                    this.pagination = {
