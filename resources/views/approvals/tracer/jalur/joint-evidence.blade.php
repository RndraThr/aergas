...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-cyan-50">
    <div class="container mx-auto px-4 py-6">
        {{-- Breadcrumb --}}
        <div class="mb-4">
            <a href="{{ route('approvals.tracer.jalur.lines', $cluster->id ?? 1) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white text-gray-700 rounded-lg hover:bg-gray-50 transition shadow-sm group">
        ...
        <div class="bg-white rounded-2xl shadow-xl border-2 border-blue-200 p-6 mb-6">
            <div class="flex items-start justify-between mb-4">
                ...
                {{-- Edit / Detail Joint Button --}}
                @if(auth()->user()->hasAnyRole(['jalur', 'admin', 'super_admin']))
                    @php
                        $canEdit = in_array($joint->status_laporan, ['draft', 'revisi_tracer', 'revisi_cgp']);
                    @endphp
                    @if($canEdit)
                        <a href="{{ route('jalur.joint.edit', $joint->id) }}"
                           class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-semibold transition shadow-sm">
                    ...
                    @else
                        <a href="{{ route('jalur.joint.show', $joint->id) }}"
                           class="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-semibold transition shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
async function approveAllPending() {
    ...
    try {
        ...
        const pendingPhotos = @json($joint->photoApprovals->whereIn('photo_status', ['tracer_pending', 'draft'])->pluck('id'));
        ...
        for (const photoId of pendingPhotos) {
            try {
                const data = await safeFetchJSON('{{ route("approvals.tracer.jalur.approve-photo") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
