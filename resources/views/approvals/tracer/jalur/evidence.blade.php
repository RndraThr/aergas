                            ...
                            @if($hasPhotos)
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    @foreach($photos as $photo)
                                        @php
                                            $isApproved = in_array($photo->photo_status, ['tracer_approved', 'cgp_approved', 'cgp_pending']);
                                            $isPending = in_array($photo->photo_status, ['tracer_pending', 'draft']);
                                            $isRejected = $photo->photo_status === 'tracer_rejected';
                                        @endphp
                                            ...
                                            <div class="p-4">
                                                ...
                                                <div class="mb-3 space-y-2">
                                                    ...
                                                    @if($isApproved)
                                                        <div>
                                                            <button type="button"
                                                                    onclick="toggleStatusDetails({{ $photo->id }}, 'approved')"
                                                    ...
                                                    @if($isRejected)
                                                        <div>
                                                            <button type="button"
                                                                    onclick="toggleStatusDetails({{ $photo->id }}, 'rejected')"
                                                                ...
                                                                <i id="rejected-chevron-{{ $photo->id }}" class="fas fa-chevron-down transition-transform"></i>
                                                            </button>
                                                            <div id="rejected-details-{{ $photo->id }}" class="hidden mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                                                                <div class="flex items-center justify-between mb-2">
