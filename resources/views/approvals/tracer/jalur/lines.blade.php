...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50" x-data="linesData()" x-init="init()">
    <div class="container-fluid px-4 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <div class="mb-3 p-3 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200">
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div>
                                        ...
                                        <span class="font-semibold text-gray-900">Ø{{ $line->diameter }}mm</span>
                                    </div>
                                    <div>
                                        <span class="text-gray-500">Status:</span>
                                        <span class="font-semibold text-gray-900">{{ $line->status_line }}</span>
                                    </div>
                                    @if($line->nama_jalan)
                                        <div class="col-span-2">
                                            <span class="text-gray-500">Jalan:</span>
                                            <span class="font-semibold text-gray-900">{{ $line->nama_jalan }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 gap-2 mb-3">
                                <div class="text-center bg-blue-50 rounded-lg p-2 border border-blue-100">
                                    ...
                                    <div class="text-sm font-bold text-blue-900">{{ number_format($line->estimasi_panjang, 1) }}m</div>
                                </div>
                                <div class="text-center bg-indigo-50 rounded-lg p-2 border border-indigo-100">
                                    ...
                                    <div class="text-sm font-bold text-indigo-900">{{ number_format($line->total_penggelaran, 1) }}m</div>
                                </div>
                                <div class="text-center bg-green-50 rounded-lg p-2 border border-green-100">
                                    ...
                                    <div class="text-sm font-bold {{ $line->actual_mc100 ? 'text-green-900' : 'text-gray-400' }}">
                                        {{ $line->actual_mc100 ? number_format($line->actual_mc100, 1) . 'm' : '-' }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3 flex-1 flex flex-col">
                                <div class="text-xs text-gray-600 mb-2 font-semibold">
                                    {{ $line->approval_stats['work_dates_count'] }} Tanggal Pekerjaan
                                </div>
                                @if(!empty($line->approval_stats['work_dates_detail']))
                                    <div class="flex-1 max-h-32 overflow-y-auto space-y-1">
                                        @foreach($line->approval_stats['work_dates_detail'] as $dateDetail)
                                            <div class="flex items-center justify-between px-2 py-1 bg-indigo-50 rounded border border-indigo-200">
                                                <span class="text-xs font-medium text-indigo-800">
                                                    {{ \Carbon\Carbon::parse($dateDetail['date'])->format('d M Y') }}
                                                </span>
                                                <span class="text-xs font-bold text-indigo-900">
                                                    {{ number_format($dateDetail['penggelaran'], 1) }}m
                                                </span>
                                            </div>
                                        @endforeach
                                    </div>
                            ...
                            <div class="mt-auto">
                                @if($line->approval_stats['total_photos'] > 0)
                                    <div class="flex items-center justify-between text-xs mb-2">
                                        <span class="text-gray-600">
                                            Photos: <span class="font-semibold text-gray-900">{{ $line->approval_stats['approved_photos'] }}/{{ $line->approval_stats['total_photos'] }}</span>
                                        </span>
                                        <span class="font-bold text-gray-900">{{ number_format($line->approval_stats['percentage'], 0) }}%</span>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                        <div class="h-2 rounded-full transition-all {{ $line->approval_stats['percentage'] === 100 ? 'bg-green-500' : 'bg-indigo-500' }}"
                                            style="width: {{ $line->approval_stats['percentage'] }}%">
                                        </div>
                                    </div>
                                @else
            ...
            @empty
                <div class="col-span-2 bg-white rounded-xl shadow-md p-12 text-center border border-gray-100">
                    ...
                    <p class="text-gray-500">
                        @if($search)
                            Tidak ditemukan line dengan kata kunci "{{ $search }}"
                        @elseif($filter !== 'all')
                            Tidak ada line dengan filter "{{ $filter }}"
                        @else
                            Belum ada line yang terdaftar di cluster ini
                        @endif
                    </p>
                </div>
            </template>
