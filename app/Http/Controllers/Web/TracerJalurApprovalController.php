...
class TracerJalurApprovalController extends Controller
{
    ...
    public function replacePhoto(Request $request)
    {
        ...
        try {
            ...
            $moduleFolder = strtoupper(str_replace('jalur_', '', $photoApproval->module_name));
            ...
            $folderPath = "aergas/JALUR_{$moduleFolder}/{$clusterSlug}/{$lineNumber}/{$date}";

            // Generate filename
            $timestamp = now()->format('Ymd_His');
            $extension = $uploadedFile->getClientOriginalExtension();
            $filename = "{$photoApproval->photo_field_name}_{$timestamp}.{$extension}";

            // Store file
            $storagePath = $uploadedFile->storeAs($folderPath, $filename, 'public');
            $photoUrl = Storage::url($storagePath);

            // Delete old file if exists
            if ($photoApproval->storage_path && Storage::disk('public')->exists($photoApproval->storage_path)) {
                Storage::disk('public')->delete($photoApproval->storage_path);
            }
