...
class SrDataController extends Controller
{
    ...
    public function index(Request $r)
    {
        ...
        if ($r->filled('tanggal_dari')) {
            $q->whereDate('tanggal_pemasangan', '>=', $r->get('tanggal_dari'));
        }

        if ($r->filled('tanggal_sampai')) {
            $q->whereDate('tanggal_pemasangan', '<=', $r->get('tanggal_sampai'));
        }

        $sr = $q->paginate((int) $r->get('per_page', 15))->withQueryString();

        // Load related data for each SR
        $sr->load('createdBy:id,name', 'photoApprovals');

        // Add photo status details for each SR
        $sr->getCollection()->transform(function ($item) {
    ...
    public function store(Request $r)
    {
        ...
        if ($existingSr) {
            return response()->json([
                'success' => false,
                'message' => 'SR untuk reff_id ' . $r->reff_id_pelanggan . ' sudah ada.',
                'existing_id' => $existingSr->id,
                'existing_status' => $existingSr->status
            ], 422);
        }
    ...
    public function getRejectionDetails(SrData $sr)
    {
        try {
            ...
            Log::error('Get SR rejection details failed', [
                'sr_id' => $sr->id,
                'error' => $e->getMessage()
            ]);
     ...
     */
    private function getPhotoStatusDetails(SrData $sr): array
    {
        $requiredPhotos = $sr->getRequiredPhotos();
        $photoApprovals = $sr->photoApprovals;
        $statusDetails = [];
    ...
    private function getPhotoLabel(string $fieldName): string
    {
        $labels = [
            ...
            'keterangan_lingkungan' => 'Keterangan Lingkungan',
            'foto_regulator' => 'Foto Regulator',
            'foto_meteran' => 'Foto Meteran',
            'scan_ktp' => 'Scan KTP',
            'scan_kk' => 'Scan KK',
        ];
